...
void blendInterval(unsigned int* fbi, int x0, int x1, float h0, float h1, int stepx, int other, int sz, bool ushape) {
  ...
  if (h0 == 0) {
    ...
    if (x0 == middle) {
      ...
      massert(0 <= x0 + other && x0 + other < sz);
  ...
  do {
    massert(0 <= x0 + shift && x0 + shift < sz);
    fbi[x0 + shift] = mixColor(area, fbi[x0], 1-area, fbi[x0 + other]);
    area += dh1;
    x0 += stepx;
    massert((0 <= area && area <= 1) || x0 > x1);
  } while (x0 <= x1);
...
//_INLINE
void MLAA(unsigned int* fbi, unsigned int* fb0, int resX, int resY, RTTL::AtomicCounter& job, int threadID = 0) {
  // Morphological AA --
  ...
  // I am so lazy...
  //massert(resX%rows_per_job == 0 && resY%rows_per_job == 0);
 ...
 main_blending_loop:
  {
    ...
    for (int yc = yfrst; yc < ylast; yc += stepy, befor = -stepy) {
      ...
      while (len = findSeparationLine(x0, x1, fb0, fc, xstart, xend, stepx)) {
        massert(x1 - x0 == (len-1)*stepx);

        if (len == 1) {
          // One cell.
          const float weightc = 7.0f/8;
          massert(0 <= x0 && x0 < resX*resY);
          fbi[x0] = mixColor(weightc, fbi[x0], 1-weightc, fbi[x0 + after]);
          massert(0 <= x0 + after && x0 + after < resX*resY);
